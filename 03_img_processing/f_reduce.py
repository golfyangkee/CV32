'''
cv2.reduce() 함수는 행 또는 열의 차원을 줄여주는 작업을 수행합니다. (전체 이미지를 줄여준다.)
...
# 랜덤으로 3x3 크기의 행렬을 생성
src = np.random.randint(0, 10, (3, 3)).astype(np.float32) # 3*3
print("Original Matrix:")
print(src)

# 행을 더하는 작업
reduce_row_sum = cv2.reduce(src, dim=0, rtype=cv2.REDUCE_SUM) # dim=0 이면 행, 1이면 열 , rtype sum이냐 avg
print("Row sum:")
