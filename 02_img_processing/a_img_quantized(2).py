...
# 양자화 -> 색상 감소 -> 이미지 파일 크기가 준다 [ 노이즈 감소 확인 : 경계선 뚜렷하게 나와 확인에 용이] , 저장 공간 절약
# 양자화하는 이유 --> 양자화를 통해 생성된 제한된 색상 집합은 색상 팔레트로 사용되며 웹, 그래픽, 게임 개발에 사용된다.

# << 양자화 후 저장된 파일의 크기, 색상 정보 등을 파일로 저장 후 출력해보자 >>
...
# 실제로는 없어서 공식을 구한다. 회사마다 만들어놨다.
# 작은 픽셀은 더 작고 촘촘하게 양자화가 되고 , 큰 픽셀은 더 크고 넓은 간격으로 양자화가 된다.
