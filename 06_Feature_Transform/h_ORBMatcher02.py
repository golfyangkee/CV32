...
#BFMatcher  :  특징이 얼마나 유사한지를 확인  ->  두이미지의 특징점이 무차별 대입 방식  [임계값 없다]
#FLANN  :    최근접 이웃 탐색 알고리즘을 사용   -> 무작위 트리 기반의 접근 방법 사용
# 이미지를 읽어온다.
...
###############################################################
# FlannBasedMatcher  객체를 생성하고, 키포인트 간의 거리를 측정한다.
...
print(matches)
#########################################################
# 매칭 결과를 거리에 따라 정렬한다.  	distance   imgIdx   queryIdx  trainIdx
